...
- `FloatingBubbles` (`src/components/FloatingBubbles.jsx`, mounted only on `Landing`'s hero) renders 10 translucent circles that drift upward on a loop (`bubbleFloat` keyframe, `index.css`) with randomized size/speed/delay/horizontal drift per bubble. Clicking one pops it into 8 small `bubble-shard` particles that scatter radially and fade (`bubbleShard` keyframe), then the bubble is removed and a fresh randomized one respawns after a short delay — keeps the count roughly steady without a fixed grid feel.
