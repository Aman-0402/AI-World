...
- All Download buttons (`ResourceCard`, `FileStep` in `StepList.jsx`) share one `DownloadButton` component: a gradient pill that morphs into a spinning-ring circle icon-button on hover, built entirely with Tailwind (`before:` pseudo-element utilities + `animate-spin`) — no styled-components/CSS-in-JS, consistent with the project's Tailwind-only rule.
